<script setup lang="ts">
  import Logo from './Logo.vue';

  import { ref } from 'vue';
  import { toggleMobileNav } from '../animations';

  const isOpen = ref(false);

  function toggleOpen() {
    isOpen.value = !isOpen.value;
    toggleMobileNav(isOpen.value);
  }
</script>
<template>
  <div class="top-nav fixed top-0 left-0 w-full h-16 font-oswald border-b border-gray-800 bg-bg-primary z-10">
    <div
      id="overlay"
      class="fixed top-0 right-0 bg-bg-secondary z-20 w-full h-screen translate-x-[100%]"
    >
      <ul
        class="flex flex-col justify-center items-center gap-y-[20px] fixed top-0 left-0 text-4xl text-primary-white h-screen w-full"
      >
        <li @click="toggleOpen">
          <a
            href="#techs"
            class="hover:text-gray-200 transition-colors"
            >techs</a
          >
        </li>
        <li @click="toggleOpen">
          <a
            href="#experience"
            class="hover:text-gray-200 transition-colors"
            >experience</a
          >
        </li>
        <li @click="toggleOpen">
          <a
            href="#projects"
            class="hover:text-gray-200 transition-colors"
            >projects</a
          >
        </li>
        <li @click="toggleOpen">
          <a
            href="#contact"
            class="hover:text-gray-200 transition-colors"
            >contact</a
          >
        </li>
        <li @click="toggleOpen">
          <a
            href="./kevin_jordan.pdf"
            target="_blank"
            rel="noopener noreferrer"
            ><button
              class="border-2 border-gray-200 p-4 rounded-md hover:bg-bg-tertiary text-primary-white transition-colors"
            >
              RESUME
            </button></a
          >
        </li>
      </ul>
    </div>
    <nav class="flex justify-between px-4 items-center h-full container mx-auto">
      <div class="nav-left">
        <h1
          id="logo"
          class="text-primary-white text-2xl font-geist"
        >
          <Logo />
        </h1>
      </div>
      <div
        class="nav-right z-30 cursor-pointer"
        @click="toggleOpen"
      >
        <svg
          viewBox="0 0 90 70"
          width="40"
          height="40"
          fill="currentColor"
          class="text-primary-white"
        >
          <rect
            id="upper"
            x="15"
            width="75"
            height="6"
            rx="8"
          ></rect>
          <rect
            id="middle"
            x="40"
            y="25"
            width="50"
            height="6"
            rx="8"
          ></rect>
          <rect
            id="lower"
            x="65"
            y="50"
            width="25"
            height="6"
            rx="8"
          ></rect>
        </svg>
      </div>
    </nav>
  </div>
</template>
